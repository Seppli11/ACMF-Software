...
import ch.sebi.acmf.undomanager.SongAddAction;
import ch.sebi.acmf.undomanager.SongChangeOrderAction;
import ch.sebi.acmf.undomanager.SongRemoveAction;
import ch.sebi.acmf.undomanager.TemplateAddAction;
import ch.sebi.acmf.undomanager.TemplateRemoveAction;
...
public class ACMFMainController {
	...
	private void addTemplateBtn() {
		...
		if (result.isPresent()) {
			...
			// ChangeManager.MAIN.add(new ListAddChange<>(timeline_panel.getTemplates(),
			// t));
		}
	}
	...
	private void removeTemplateBtn() {
		UndoManager.MAIN.add(new TemplateRemoveAction(song_listview.getSelectionModel().getSelectedItem(),
				timeline_panel.getSelectedTemplate()));
		timeline_panel.removeSelectedNode();
	}

	private Dialog<Pair<String, Pair<DeviceConfiguration, DeviceConfiguration>>> getSongDialog(String name,
			DeviceConfiguration dc1, DeviceConfiguration dc2, boolean editDC) {
		...
		createButtonN.setDisable(!(!songName.getText().trim().isEmpty() && !dc1CB.getSelectionModel().isEmpty()
				&& !dc2CB.getSelectionModel().isEmpty()));
	...
	private void addSongBtn() {
		Optional<Pair<String, Pair<DeviceConfiguration, DeviceConfiguration>>> result = getSongDialog("", null, null,
				true).showAndWait();
		result.ifPresent(stringPairPair -> {
			if (stringPairPair.getKey() == null) {
				return;
			}
			Song s = new Song(result.get().getKey(), result.get().getValue().getKey(),
	...
	private void removeSongBtn() {
		if (song_listview.getSelectionModel().getSelectedItem() != null) {
			Song songToRemove = song_listview.getSelectionModel().getSelectedItem();
			// ChangeManager.MAIN.add(new ListRemoveChange<Song>(song_listview.getItems(),
			// songToRemove));
	...
	private void editSongBtn() {
		if (song_listview.getSelectionModel().getSelectedItem() != null) {
			...
			Optional<Pair<String, Pair<DeviceConfiguration, DeviceConfiguration>>> result = getSongDialog(s.getName(),
					s.getMidi1DeviceConfiguration(), s.getMidi2DeviceConfiguration(), false).showAndWait();
			result.ifPresent(stringPairPair -> {
				if (result.get().getKey() == null || result.get().getValue().getKey() == null
						|| result.get().getValue().getValue() == null) {
					return;
				}
	...
	private void moveSongDownBtn() {
		if (song_listview.getSelectionModel().getSelectedItem() != null) {
			...
			int newIndex = index + 1;
			if (newIndex >= song_listview.getItems().size()) {
				newIndex = song_listview.getItems().size();
			}
	...
	private void saveSongs() {
		...
		SongSet.save(ss, songDirectory);

		// build and show save notification
		Alert alert = new Alert(AlertType.INFORMATION);
		alert.setContentText("Das Project wurde gespeichert.");
		alert.setTitle("Projekt gespeichert");
		alert.show();
	}
	 ...
	 * @param t the throable/exception
	 */
	public static void showErrorDialog(Throwable t) {
		//new ExceptionDialog(t).showAndWait();
	}
}
