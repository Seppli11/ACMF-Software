...
public class DeviceConfigurationManager {
	...
	static {
		logger  = LogManager.getLogger(DeviceConfigurationManager.class.getName());
	...
	public static ObservableList<DeviceConfiguration> readDeviceConfigDir() {
		...
		logger.info("Loaded DeviceConfigurations:");
	...
	private static DeviceConfiguration toDeviceConfiguration(JSONObject obj) {
		if(!obj.has("name")) {
			throw new IllegalStateException("Attribute name is not in the JSONObject (JSONObject: '" + obj.toString() + "')");
		}
		if(!obj.has("values")) {
			throw new IllegalStateException("Attribute name is not in the JSONObject (JSONObject: '" + obj.toString() + "')");
		}
		...
		for(int i1 = 0; i1 < values.length(); i1++) {
			...
			if(!valueJsonObj.has("name")) {
				throw new IllegalStateException("Attribute name is not in the JSONObject (JSONObject: '" + valueJsonObj.toString() + "')");
			}
			if(!valueJsonObj.has("type")) {
				throw new IllegalStateException("Attribute type is not in the JSONObject (JSONObject: '" + valueJsonObj.toString() + "')");
			}
			if(!valueJsonObj.has("msg")) {
				throw new IllegalStateException("Attribute msg is not in the JSONObject (JSONObject: '" + valueJsonObj.toString() + "')");
			}
			...
			if(type == DeviceConfiguration.TValueType.Int) {
				if(!valueJsonObj.has("min")) {
					throw new IllegalStateException("Attribute min is not in the JSONObject (JSONObject: '" + valueJsonObj.toString() + "')");
				}
				if(!valueJsonObj.has("max")) {
					throw new IllegalStateException("Attribute max is not in the JSONObject (JSONObject: '" + valueJsonObj.toString() + "')");
				}
				...
				if(!valueJsonObj.has("enums")) {
					throw new IllegalStateException("Attribute enums is not in the JSONObject (JSONObject: '" + valueJsonObj.toString() + "')");//checks if enums attribute is there
				}
				...
				for(int i2 = 0; i2 < enumsArray.length(); i2++) {
					...
					if(!enumsJsonObj.has("name")) {
						throw new IllegalStateException("Attribute name is not in the JSONObject (JSONObject: '" + enumsJsonObj.toString() + "')");
					}
					if(!enumsJsonObj.has("value")) {
						throw new IllegalStateException("Attribute value is not in the JSONObject (JSONObject: '" + enumsJsonObj.toString() + "')");
					}
